...
if (isset($_POST["register"])) {
    ...
    $usernameCheckQuery = $user->usernameAvailability($USERNAME);
    $emailCheckQuery = $user->emailAvailability($EMAIL);
    $numUser = mysqli_num_rows($usernameCheckQuery);
    $numEmail = mysqli_num_rows($usernameCheckQuery);
    if ($numUser > 0) {
        echo "<script>alert('Username already in use')</script>";
    } elseif ($numEmail > 0) {
        echo "<script>alert('Email address already in use')</script>";
    } else {
        $userRegistration = $user->registration($FIRSTNAME, $LASTNAME, $USERNAME, $EMAIL, $PASSWORD);
        if ($userRegistration) {
            // Message for successful registration
            echo "<script>alert('User registration successful.');</script>";
            echo "<script>window.location.href='login.php'</script>";
        } else {
            // Message for unsuccessful registration
            echo "<script>alert('Not successful. Something went wrong. Please try again');</script>";
